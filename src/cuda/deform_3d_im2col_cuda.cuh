...
__device__ scalar_t dmcn_3d_im2col_telinear(const scalar_t *bottom_data, const int data_height, const int data_width,
                                      const int depth, const int height, const int width, scalar_t d, scalar_t h, scalar_t w)
{
  ...
  if (d_low >= 0 && h_low >= 0 && w_low >= 0)
    v1 = bottom_data[d_low * data_height * data_width + h_low * data_width + w_low];
  scalar_t v2 = 0;
  if (d_low >= 0 && h_low >= 0 && w_high <= width - 1)
    v2 = bottom_data[d_low * data_height * data_width + h_low * data_width + w_high];
  scalar_t v3 = 0;
  if (d_low >= 0 && h_high <= height - 1 && w_low >= 0)
    v3 = bottom_data[d_low * data_height * data_width + h_high * data_width + w_low];
  scalar_t v4 = 0;
  if (d_low >= 0 && h_high <= height - 1 && w_high <= width - 1)
    v4 = bottom_data[d_low * data_height * data_width + h_high * data_width + w_high];
  scalar_t v5 = 0;
  if (d_high <= depth - 1 && h_low >= 0 && w_low >= 0)
    v5 = bottom_data[d_high * data_height * data_width + h_low * data_width + w_low];
  scalar_t v6 = 0;
  if (d_high <= depth - 1 && h_low >= 0 && w_high <= width - 1)
    v6 = bottom_data[d_high * data_height * data_width + h_low * data_width + w_high];
  scalar_t v7 = 0;
  if (d_high <= depth - 1 && h_high <= height - 1 && w_low >= 0)
    v7 = bottom_data[d_high * data_height * data_width + h_high * data_width + w_low];
  scalar_t v8 = 0;
  if (d_high <= depth - 1 && h_high <= height - 1 && w_high <= width - 1)
    v8 = bottom_data[d_high * data_height * data_width + h_high * data_width + w_high];
...
template <typename scalar_t>
__device__ scalar_t dmcn_3d_get_gradient_weight(scalar_t argmax_d, scalar_t argmax_h, scalar_t argmax_w,
...
template <typename scalar_t>
__device__ scalar_t dmcn_3d_get_coordinate_weight(scalar_t argmax_d, scalar_t argmax_h, scalar_t argmax_w,
...
template <typename scalar_t>
__global__ void deformable_3d_im2col_gpu_kernel(const int n,
                                                       ...
                                                       scalar_t *data_col)
{
  ...
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    // here columns is of shape (c*kd*kh*kw, N, od, oh, ow), need to adapt axis
    ...
    for (int i = 0; i < kernel_d; ++i)
    {
      for (int j = 0; j < kernel_h; ++j)
      {
        for (int k = 0; k < kernel_w; ++k)
        {
          ...
          {
            val = dmcn_3d_im2col_telinear(data_im_ptr, height, width, depth, height, width, d_im, h_im, w_im);
          }
          *data_col_ptr = val;
...
template <typename scalar_t>
__global__ void deformable_3d_col2im_gpu_kernel(const int n,
                                                       const scalar_t *data_col, const scalar_t *data_offset, const int channels, 
                                                       const int depth, const int height, const int width,
                                                       const int kernel_d, const int kernel_h, const int kernel_w,
                                                       const int pad_d, const int pad_h, const int pad_w,
                                                       const int stride_d, const int stride_h, const int stride_w,
                                                       ...
                                                       scalar_t *grad_im)
{
  CUDA_KERNEL_LOOP(index, n)
  {
    const int k = (index / width_col / height_col / depth_col / batch_size) % kernel_w;
    const int j = (index / width_col / height_col / depth_col / batch_size / kernel_w) % kernel_h;
    const int i = (index / width_col / height_col / depth_col / batch_size / kernel_w / kernel_h) % kernel_d;
    const int c = index / width_col / height_col / depth_col / batch_size / kernel_w / kernel_h / kernel_d;
      ...
      for (int dy = -2; dy <= 2; dy++)
      {
        for (int dx = -2; dx <= 2; dx++)
        {
              ...
              abs(cur_inv_w_data - (cur_w + dx)) < 1)
          {
            ...
            scalar_t weight = dmcn_3d_get_gradient_weight(cur_inv_d_data, cur_inv_h_data, cur_inv_w_data, cur_d + dz, cur_h + dy, cur_w + dx, depth, height, width);
...
template <typename scalar_t>
__global__ void deformable_3d_col2im_coord_gpu_kernel(const int n,
                                                             ...
                                                             scalar_t *grad_offset)
{
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    int cnt = 0;
    const scalar_t *data_col_ptr = data_col + deformable_group_index * channel_per_deformable_group * batch_size * width_col * height_col * depth_col;
    ...
    for (int col_c = (offset_c / 3); col_c < channel_per_deformable_group; col_c += col_step)
    {
      ...
      const scalar_t weight = dmcn_3d_get_coordinate_weight(
          inv_d, inv_h, inv_w,
...
void deformable_3d_im2col_cuda(cudaStream_t stream,
  ...
  deformable_3d_im2col_gpu_kernel<scalar_t>
...
void deformable_3d_col2im_cuda(cudaStream_t stream,
  ...
  deformable_3d_col2im_gpu_kernel<scalar_t>
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS,
          0, stream>>>(
...
void deformable_3d_col2im_coord_cuda(cudaStream_t stream,
  ...
  deformable_3d_col2im_coord_gpu_kernel<scalar_t>
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS,
        0, stream>>>(
