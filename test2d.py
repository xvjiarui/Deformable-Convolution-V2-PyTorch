...
def check_mdconv_zero_offset():
    ...
    dcn = ModulatedDeformConv2d(inC, outC, (kH, kW),
                   stride=1, padding=1, dilation=1,
                   groups=2, 
                   deformable_groups=deformable_groups, im2col_step=1).cuda()
    pcn = nn.Conv2d(inC, outC, (kH, kW), stride=1, padding=1, dilation=1, groups=2).cuda()
...
def check_dconv_im2col_step_forward():
    ...
    dcn1 = DeformConv2d(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=1).cuda()

    dcn2 = DeformConv2d(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=2).cuda()
...
def check_mdconv_im2col_step_forward():
    ...
    dcn1 = ModulatedDeformConv2d(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=1).cuda()

    dcn2 = ModulatedDeformConv2d(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=2).cuda()
...
def check_dconv_im2col_step_backward():
    ...
    output1 = _DeformConv2d(input, offset, weight, bias, stride, padding, dilation, groups, deformable_groups, 2)
    ...
    output2 = _DeformConv2d(input, offset, weight, bias, stride, padding, dilation, groups, deformable_groups, 1)
...
def check_mdconv_im2col_step_backward():
    ...
    output1 = _ModulatedDeformConv2d(input, offset, mask, weight, bias, stride, padding, dilation, groups, deformable_groups, 2)
    targert = torch.rand(*output1.size()).cuda()
    error = (targert - output1).mean()
    error.backward(retain_graph=True)
    input_grad = input.grad.clone()
    ...
    weight_grad = weight.grad.clone()
    bias_grad = bias.grad.clone()
    output2 = _ModulatedDeformConv2d(input, offset, mask, weight, bias, stride, padding, dilation, groups, deformable_groups, 1)
    error2 = (targert - output2).mean()
    error.backward()
    print((output1 - output2).abs().max())
    input_grad_err = (input.grad - 2 * input_grad).abs().max() 
...
def check_gradient_mdconv():
    ...
    print('check_gradient_mdconv: ',
          gradcheck(_ModulatedDeformConv2d, (input, offset, mask, weight, bias,
                    stride, padding, dilation, groups, deformable_groups, im2col_step),
                    eps=1e-3, atol=1e-3, rtol=1e-2, raise_exception=True))
    # print('check_gradient_mdconv: ',
...
def example_dconv():
    ...
    dcn = DeformConv2dPack(64, 128, kernel_size=(3, 3), stride=1,
...
def example_mdconv():
    input = torch.randn(2, 64, 128, 128).cuda()
    # wrap all things (offset and mask) in DCN
    dcn = ModulatedDeformConv2dPack(64, 128, kernel_size=(3, 3), stride=1,
